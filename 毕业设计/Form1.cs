...
namespace 毕业设计
{
    public partial class Form1 : Form
    {
        ...
        private Bitmap _currentBitMap = new Bitmap(2000,1000);//记录当前操作产生的BitMap

        private void 生成底图结构ToolStripMenuItem_Click_1(object sender, EventArgs e)//10分格的底图结构生成
        {
            //生成10分格底图结构
            this.panelofPic.SetAutoScrollMargin(1000, 100) ;
            ...
            this._currentBitMap = trainBasicPicture10.DrawTrainBasicPicture(TrainBasicPicture.Scale.TenMinutes);
            pictureBox1.BackgroundImage =this._currentBitMap;//将返回的bitmap设置为picbox的背景图片
        }
        ...
        {
            //生成5分格的底图结构
            this.panelofPic.SetAutoScrollMargin(3000, 100);
            ...
            this._currentBitMap = trainBasicPicture5.DrawTrainBasicPicture(TrainBasicPicture.Scale.FiveMinutes);//绘图并且存储
            pictureBox1.BackgroundImage = this._currentBitMap;//将返回的bitmap设置为picbox的背景图片
        }
        ...
        {
            //生成5分格的底图结构
            this.panelofPic.SetAutoScrollMargin(800, 100);
            TrainBasicPicture trainBasicPicture5 = new TrainBasicPicture();//声明5分格底图结构实例
            trainBasicPicture5.Width = this.panelofPic.Width * 2;
            trainBasicPicture5.Height = this.panelofPic.Height;
            trainBasicPicture5.TrainBasicPicturePos = new PointF(this.pictureBox1.Location.X + 10, this.pictureBox1.Location.Y + 10);
